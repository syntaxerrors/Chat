...
class UpdateChatCommand extends Command {
	 ...
	 * @var string
	 */
	protected $name = 'syntax:update-chat';
	...
	{
		// Handle Chat
		if ($this->confirm('Do you want to update Syntax\Chat? [yes|no]')) {
			$requestedVersion = $this->ask('What version of chat do you require?  [Hit enter to leave as dev-master]', 'dev-master');

			$this->comment('Running composer update...');
			$this->composerUpdate($requestedVersion);

			$currentVersion = Config::get('chat::version');
			$newVersion     = Syntax\Chat\ChatServiceProvider::version;
	...
	protected function composerUpdate($version)
	{
		$commands = [
			'cd '. base_path(),
			'composer update syntax/chat:'. $version,
		];
	...
	{
		list($path, $contents) = $this->getConfig('packages/syntax/chat/config.php');

		$contents = str_replace($this->laravel['config']['chat::version'], $version, $contents);
